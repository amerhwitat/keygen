...
public final class DesktopStartupMenu {
    ...
    public DesktopStartupMenu(DesktopProfileRegistry registry, Set<DesktopCapability> capabilities) {
        this.registry = registry; this.capabilities = Set.copyOf(capabilities);
    }
    ...
    public DesktopLaunchPlan select(String id) {
        ...
        if (!p.recoveryProfile() && !available(p)) throw new DesktopSelectionException("desktop profile unavailable: " + id);
        var candidate = p.commandCandidates().stream().filter(c -> !c.isEmpty()).findFirst()
                .orElseThrow(() -> new DesktopSelectionException("profile has no launcher: " + id));
