...
public final class DesktopResource {
    ...
    public DesktopResource() {
        ...
        var detector = new DesktopCapabilityDetector();
        var capabilities = detector.detect(env);
        this.menu = new DesktopStartupMenu(registry, capabilities,
                command -> detector.launcherAvailable(command, env));
    }

    @GET @Path("/profiles")
    public List<DesktopStartupMenu.Entry> profiles() { return menu.entries(); }
    ...
    @GET @Path("/select/{id}")
    public DesktopLaunchPlan select(@PathParam("id") String id) {
